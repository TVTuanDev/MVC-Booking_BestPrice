...
namespace Booking.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult FlightOrder()
        {
            return View();
        }
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                foreach (var flight in bestFlights ?? new JArray())
                {
                    var flightData = new FlightVM
                    {
                        ...
                        Price = Math.Round(decimal.Parse(flight["price"]?.ToString() ?? "0") / 1000) * 1000,
        ...
        public async Task<JsonResult> TourFilterHandle(string filter)
        {
            ...
            var listFilter = filter.Split(";");

            // Lấy slug từ mảng đầu tiên
            var slug = listFilter.First().Split(":").Last();
            var cateTour = await _context.CategoryTours.FirstOrDefaultAsync(ch => ch.Slug == slug);
            ...
            var listTourFilter = new List<Tour>();
            ...
            foreach (var itemFilter in listFilter)
            {
                ...
                {
                    var tours = listTour.Where(h => h.Departure == item.Last()).ToList();
                    listTourFilter.AddRange(tours);
                }
                if (item.First() == "Duration")
                {
                    var tours = listTour.Where(h => h.Duration == item.Last()).ToList();
                    listTourFilter.AddRange(tours);
                }
                if (item.First() == "Price")
                {
                    if (item.Last() == "0")
                    {
                        var tours = listTour.Where(h => h.Price >= 1000000 && h.Price < 5000000).ToList();
                        listTourFilter.AddRange(tours);
                    }
                    if (item.Last() == "1")
                    {
                        var tours = listTour.Where(h => h.Price >= 5000000 && h.Price < 10000000).ToList();
                        listTourFilter.AddRange(tours);
                    }
                    if (item.Last() == "2")
                    {
                        var tours = listTour.Where(h => h.Price >= 10000000 && h.Price < 15000000).ToList();
                        listTourFilter.AddRange(tours);
                    }
                    if (item.Last() == "3")
                    {
                        var tours = listTour.Where(h => h.Price >= 15000000).ToList();
                        listTourFilter.AddRange(tours);
                    }
                }
            }
        ...
        public async Task<JsonResult> HotelFilterHandle(string filter)
        {
            ...
            var listHotelFilter = new List<Hotel>();
            ...
            foreach (var itemFilter in listFilter)
            {
                ...
                {
                    var hotels = listHotel.Where(h => h.ResidenceType == item.Last()).ToList();
                    listHotelFilter.AddRange(hotels);
                }
                if (item.First() == "Star")
                {
                    var hotels = listHotel.Where(h => h.Star == int.Parse(item.Last())).ToList();
                    listHotelFilter.AddRange(hotels);
                }
                if (item.First() == "Price")
                {
                    if (item.Last() == "0")
                    {
                        var hotels = listHotel.Where(h => h.Rooms.Any(r => r.Price < 500000)).ToList();
                        listHotelFilter.AddRange(hotels);
                    }
                    if (item.Last() == "1")
                    {
                        var hotels = listHotel.Where(h => h.Rooms.Any(r => r.Price >= 500000 && r.Price < 1000000)).ToList();
                        listHotelFilter.AddRange(hotels);
                    }
                    if (item.Last() == "2")
                    {
                        var hotels = listHotel.Where(h => h.Rooms.Any(r => r.Price >= 1000000 && r.Price < 2000000)).ToList();
                        listHotelFilter.AddRange(hotels);
                    }
                    if (item.Last() == "3")
                    {
                        var hotels = listHotel.Where(h => h.Rooms.Any(r => r.Price >= 2000000 && r.Price < 3000000)).ToList();
                        listHotelFilter.AddRange(hotels);
                    }
                    if (item.Last() == "4")
                    {
                        var hotels = listHotel.Where(h => h.Rooms.Any(r => r.Price >= 3000000)).ToList();
                        listHotelFilter.AddRange(hotels);
                    }
